...
const VideoJsPlayer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Only initialize video.js when component has mounted and URL is available
    if (!videoUrl) return;
    
    // Short timeout to ensure DOM is fully rendered
    const timeoutId = setTimeout(() => {
      if (containerRef.current && videoRef.current) {
        console.log('Initializing VideoJS with URL:', videoUrl);
        ...
        try {
          ...
          player.on('error', () => {
            ...
            setError(`Video error: ${error?.code || 'unknown'} - ${error?.message || 'Unknown error'}`);
            setLoading(false);
          });
